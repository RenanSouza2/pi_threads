...
void binary_splitting(union_num_t out[], uint64_t size, uint64_t i_0, uint64_t i_max)
{
    ...
    out[0] = union_num_mul(res_1[0], res_2[0]);
    out[1] = union_num_mul(res_1[1], res_2[1]);
    out[2] = union_num_add(u_r_1, u_r_2);
    return;
}

// out vector length 2, returns Q, R in that order
void binary_splitting_qr(union_num_t out[], uint64_t size, uint64_t i_0, uint64_t i_max)
{
    assert(i_0 <= i_max);
    if(i_0 == i_max)
    {
        ...
        out[0] = union_num_wrap_sig(sig_q, size);
        out[1] = union_num_wrap_sig(sig_r, size);
        return;
    }

    uint64_t i_half = (i_0 + i_max) / 2;
    union_num_t res_1[3], res_2[2];
    binary_splitting(res_1, size, i_0       , i_half);
    binary_splitting_qr(res_2, size, i_half + 1, i_max);

    union_num_t u_r_1 = union_num_mul(res_1[2], union_num_copy(res_2[0]));
    union_num_t u_r_2 = union_num_mul(res_2[1], union_num_copy(res_1[0]));

    out[0] = union_num_mul(res_1[1], res_2[0]);
    out[1] = union_num_add(u_r_1, u_r_2);
    return;
}
...
float_num_t pi_v3(uint64_t size)
{
    ...
    union_num_t res[3];
    binary_splitting_qr(res, size, 1, index_max);

    float_num_t flt_pi = union_num_unwrap_float(res[1]);
    flt_pi = float_num_mul_sig(flt_pi, sig_num_wrap(6));
    flt_pi = float_num_div(flt_pi, union_num_unwrap_float(res[0]));
